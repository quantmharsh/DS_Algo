...
public:
    ...
    void edit(int taskId, int newPriority) {
        //push fresh priority into maxheap
        maxHeap.push({newPriority , taskId});
        //update priority in tasktopriority map
    ...
    int execTop() {
        while(!maxHeap.empty())
        {
                ...
                maxHeap.pop();
                if(topElement.first == taskToPriority[topElement.second])
                {
                    taskToPriority[topElement.second]=-1;
                    return taskToUser[topElement.second];
                }
        }
        return -1;
